...
/* This program reads planar graphs and filters those out that are balanced.
 * 
 * 
 * Compile with:
 *     
 *     cc -o filter_balanced -O4 filter_balanced.c
 * 
 */
...
boolean isFaceAlternating() {
    ...
    for(i=0; i<nf; i++){
        e = elast = facestart[i];
        do {
            if (faceSize[i]==faceSize[e->inverse->rightface]) {
                return FALSE;
            }
            e = e->inverse->prev;
        } while (e != elast);
...
boolean isAlternating(){
    return isVertexAlternating() && isFaceAlternating();
}
...
boolean isVertexBalanced(){
    ...
    for(i=0; i<nv; i++){
        ...
        if (degree[i]==4 && deg3Neighbours!=2){
            return FALSE;
        } else if (degree[i]==5 && !(deg3Neighbours==2 || deg3Neighbours==3)){
            return FALSE;
        }
    }
...
boolean isFaceBalanced(){
    ...
    for(i=0; i<nf; i++){
        ...
        e = elast = facestart[i];
        int size3Neighbours = 0;
        do {
            if (faceSize[e->inverse->rightface]==3) {
                size3Neighbours++;
            }
            e = e->inverse->prev;
        } while (e != elast);
        if (faceSize[i]==4 && size3Neighbours!=2){
            return FALSE;
        } else if (faceSize[i]==5 && !(size3Neighbours==2 || size3Neighbours==3)){
            return FALSE;
        }
    }
...
boolean isBalanced(){
    if(isAlternating() && is345Apg()){
        if (onlyVertex){
            return isVertexBalanced();
        } else if (onlyFace){
            return isFaceBalanced();
        } else {
            return isVertexBalanced() && isFaceBalanced();
        }
    } else {
        return FALSE;
    }
...
void help(char *name) {
    fprintf(stderr, "The program %s reads planar graphs and filters those out that are balanced.\n\n", name);
    ...
    fprintf(stderr, "       Only check for being vertex-balanced.\n");
    fprintf(stderr, "    -f, --face\n");
    fprintf(stderr, "       Only check for being face-balanced.\n");
...
int main(int argc, char *argv[]) {
    ...
    fprintf(stdout, "Read %d graph%s of which %d %s %sbalanced.\n",
            numberOfGraphs, numberOfGraphs==1 ? "" : "s",
            numberOfBalanced, numberOfBalanced==1 ? "was" : "were",
            onlyVertex ? "vertex-" : (onlyFace ? "face-" : ""));
