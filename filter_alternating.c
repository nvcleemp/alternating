...
boolean isAllowedGraph(){
    if (onlyVertex){
        return isVertexAlternating();
    } else if (onlyFace){
        return isFaceAlternating();
    } else {
...
int main(int argc, char *argv[]) {
    ...
    fprintf(stdout, "Read %d graph%s of which %d %s %salternating.\n",
            numberOfGraphs, numberOfGraphs==1 ? "" : "s",
            numberOfAlternating, numberOfAlternating==1 ? "was" : "were",
